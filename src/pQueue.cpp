...
void priorityQueue::minHeapify(int pos){
	...
	while(swapped && i < _size){
		swapped = false;
		minPos = i;
		minValue = (*_ref)[_vector[i]];
		if(left_child(i) < _size && (*_ref)[_vector[left_child(i)]] < minValue){
			minPos = left_child(i);
			minValue = (*_ref)[_vector[left_child(i)]];
		}
		if(right_child(i) < _size && (*_ref)[_vector[right_child(i)]] < minValue){
			minPos = right_child(i);
			minValue = (*_ref)[_vector[right_child(i)]];
		}
		if(minPos != i){
			swap(i, minPos);
			swapped = true;
...
void priorityQueue::moveUp(int pos){
	while(parent(pos) > 0 && (*_ref)[pos] < (*_ref)[parent(pos)]){
		swap(pos, parent(pos));
		pos = parent(pos);
	}
}
...
priorityQueue::priorityQueue(int D, int **ref){ // construtor
	_maxSize = D * D;
	_ref = ref;
